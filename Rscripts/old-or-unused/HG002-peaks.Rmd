...
hg_38_peaks = bind_rows(list(
    paternal = h1,
    maternal = h2,
    paternal_low_cov = h1_low_cov,
    maternal_low_cov = h2_low_cov
    ),
    .id="source"
)
...
table(hg002_dsa_only$sample_coverage)
...
gwas_long = bind_rows(
    list(
        random=random,
        GRCh38 = hg002_df %>% filter(ref == "GRCh38"),
        `GRCh38-only` = hg002_hg38_only,
        `DSA-H1` = hg_38_peaks %>% filter(grepl("paternal", source)),
        `DSA-H2` = hg_38_peaks %>% filter(grepl("maternal", source)),
    ...
    valr::bed_map(
        gwas %>% 
            expand_grid(sample_coverage=unique(hg002_df$sample_coverage)) %>%
            group_by(sample_coverage),
        n_gwas = n(),
    ) %>%
    replace_na(list(n_gwas = 0)) %>%
    group_by(source, sample_coverage) %>%
    summarise(
        n_peaks = n(),
        n_gwas = sum(n_gwas),
        n_bp = sum(end - start),
...
wide_gwas %>%
    ggplot(
        aes(
            y = source, 
            x = enrichment_per_base,
            fill = source,
            group = paste(source, sample_coverage)
        )
    ) +
    ...
    scale_x_continuous(
        name = "Fold enrichment of GWAS variants",
        trans=t_shift,
        #trans="log10",
        #limits = c(0.75,NA),
...
tmp.hap_df = hg002_df %>%
    rowwise() %>%
    mutate(
        p_value = fisher.test(matrix(c(fire_coverage_H1, coverage_H1 - fire_coverage_H1,
                                       fire_coverage_H2, coverage_H2 - fire_coverage_H2), nrow=2))$p.value,
    ) %>%
    group_by(method, group, source)
```
...
hap_df = tmp.hap_df %>%
    filter(
        !grepl("chrX", chrom),
        !grepl("chrY", chrom),
        (sample_coverage == "Low coverage" & coverage_H1 >= 10 & coverage_H2 >= 10) |
        (sample_coverage == "High coverage" & coverage_H1 >= 30 & coverage_H2 >= 30),
        pass_coverage
    ) %>%
    mutate(
...
# write the hap_df to a bed files
hap_df %>%
    rename(
        `#chrom` = chrom,
    ) %>%
    # move chrom start and end to the first three columns
    select(
        `#chrom`, start, end, everything()
    ) %>%
    fwrite("Rtables/HG002-peaks-hap-selective.bed.gz", sep="\t", quote=F, row.names=F)
```
...
hap_df %>%
    ...
    ggplot(aes(y = group, fill = method)) +
    geom_bar() +
    geom_text(
        aes(
    ...
    mrv_vgrid() +
    scale_fill_manual(values = my_colors) +
    facet_col(~sample_coverage, scales="free") +
    scale_y_discrete(
        name = "Method",
    ) +
    scale_x_continuous(
...
hap_df %>%
    ...
    facet_col(~sample_coverage, scales="free_y") +
    theme(
    )
