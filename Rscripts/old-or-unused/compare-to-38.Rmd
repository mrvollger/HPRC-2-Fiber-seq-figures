...
lift_summary <- peaks_with_38_info %>%
    group_by(ID, Sample, PS, haplotype, Technology) %>%
    summarise(
        n_peaks = n(),
        n_blacklist = sum(in_blacklist),
        frac_blacklist = n_blacklist / n_peaks,
        n_no_lift = sum(is.na(hg38_start)),
        frac_no_lift = n_no_lift / n_peaks,
        ...
        frac_unlifted = (n_peaks - n_peaks_lift_38) / n_peaks,
        frac_unlifted_that_are_sd = (n_sd_peaks - n_sd_peaks_lift_38) / (n_peaks - n_peaks_lift_38),
        test_sum = frac_lift_38 + frac_unlifted,
    ) %>%
...
lift_summary %>%
    ...
    pivot_longer(
        ...
        # cols = c(frac_no_lift, frac_unlifted_that_are_sd),
        names_to = "type",
        values_to = "value"
    ) %>%
...
# an idogram of HG002 of the unlifted peaks 
```{r}
peaks_with_38_info %>%
    filter(Sample == "HG002") %>%
    filter(!lifts_to_38) %>%
...
my_ggsave(
    "Figures/compare-to-38-HG002-unlifted-peaks-ideogram.pdf",
    width = 6,
    height = 20
)
```