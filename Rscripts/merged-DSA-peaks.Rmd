...
t_all_manifest <- read.csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vQANHblpVkMcVMWUvYas6JQl0pqvgeiYDlWwG2xV9vQdihgTsdWqHfl0z5BKb1I8hXcQtH1WIhwTkII/pub?output=csv") %>%
    filter(file.exists(dsa)) %>%
    mutate(
        has_peaks = file.exists(dsg_fire_peaks),
        has_peaks_to_chm13 = file.exists(fire_peaks_dsa_to_chm13),
        has_peaks_to_hg38 = file.exists(fire_peaks_dsa_to_hg38),
    ) %>%
    data.table()
...
all_manifest <- t_all_manifest %>%
    filter(has_peaks) %>%
    filter(has_peaks_to_chm13) %>%
    filter(has_peaks_to_hg38) %>%
    data.table()
...
if (load) {
    ...
    peaks %>%
        ...
        group_by(ID) %>%
        do({
            group_name <- unique(.$ID)
            file_name <- paste0("Tables/by-sample/", group_name, ".peaks.bed.gz")
            print(file_name)
            fwrite(., file = file_name, sep = "\t", quote = F, row.names = F)
            as.data.table(nrow(.))
        })

    # also want to save the all_manifest
    fwrite(
        all_manifest,
        "Tables/sample-and-file-manifest.tsv",
